...
window.rbi_renderTasksList = async function() {
    ...
    let globalActionsHtml = `
        <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
            ...
            <button onclick="rbi_openTaskModal()" class="bg-indigo-50 text-indigo-700 border border-indigo-200 py-3 rounded-xl font-black text-[10px] uppercase active:scale-95 shadow-sm flex items-center justify-center gap-1.5"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15"></path></svg> Вручную</button>
            <button onclick="generateAiRoutePlan()" class="bg-purple-50 text-purple-700 border border-purple-200 py-3 rounded-xl font-black text-[10px] uppercase active:scale-95 shadow-sm flex items-center justify-center gap-1.5"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg> AI-Маршрут</button>
            <button onclick="gameToggleAbsence()" class="bg-amber-50 text-amber-700 border border-amber-200 py-3 rounded-xl font-black text-[10px] uppercase active:scale-95 shadow-sm flex items-center justify-center gap-1.5"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z"></path></svg> Отпуск</button>
        </div>
    ...
    if (typeof engineerAbsence !== 'undefined' && engineerAbsence.isActive) {
        container.innerHTML = globalActionsHtml + `<div class="bg-white border border-amber-200 rounded-xl p-6 text-center text-amber-700 shadow-sm"><div class="font-black uppercase mb-1 text-lg flex items-center justify-center gap-2"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z"></path></svg> Режим: ${engineerAbsence.reason}</div>Задачи приостановлены. Хорошего отдыха!</div>`;
        return;
    }
    ...
    const renderCard = (t, isOverdue, isArchive = false) => {
        ...
        let statusBadge = '';
        if (isArchive) {
            if (t.status === 'done') statusBadge = '<div class="absolute top-2 right-2 text-green-600 font-black text-[10px] uppercase bg-green-50 px-2 py-1 rounded border border-green-200 flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"></path></svg> Выполнено</div>';
            if (t.status === 'blocked') statusBadge = '<div class="absolute top-2 right-2 text-red-600 font-black text-[10px] uppercase bg-red-50 px-2 py-1 rounded border border-red-200 flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"></path></svg> Отменено</div>';
        }
    ...
    if (activeToday.length > 0) accordionsHtml += `<details class="mb-3 group [&_summary::-webkit-details-marker]:hidden" open><summary class="cursor-pointer flex justify-between items-center mb-2 select-none border-b border-[var(--card-border)] pb-2"><span class="text-[11px] font-black text-indigo-600 uppercase tracking-widest flex items-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg> СЕГОДНЯ И ПРОСРОЧЕНО (${activeToday.length})</span><span class="text-slate-400 group-open:rotate-180">▼</span></summary><div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 pb-2">${activeToday.join('')}</div></details>`;
    
    if (weekTasks.length > 0) accordionsHtml += `<details class="mb-3 group [&_summary::-webkit-details-marker]:hidden"><summary class="cursor-pointer flex justify-between items-center mb-2 select-none border-b border-[var(--card-border)] pb-2"><span class="text-[11px] font-black text-slate-600 uppercase tracking-widest flex items-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg> ДО КОНЦА НЕДЕЛИ (${weekTasks.length})</span><span class="text-slate-400 group-open:rotate-180">▼</span></summary><div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 pb-2">${weekTasks.map(t => renderCard(t, false)).join('')}</div></details>`;
    
    if (monthTasks.length > 0) accordionsHtml += `<details class="mb-3 group [&_summary::-webkit-details-marker]:hidden"><summary class="cursor-pointer flex justify-between items-center mb-2 select-none border-b border-[var(--card-border)] pb-2"><span class="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg> БУДУЩИЕ ЗАДАЧИ (${monthTasks.length})</span><span class="text-slate-400 group-open:rotate-180">▼</span></summary><div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 pb-2">${monthTasks.map(t => renderCard(t, false)).join('')}</div></details>`;
    
    if (archiveTasks.length > 0) {
        const recentArchive = archiveTasks.sort((a,b) => new Date(b.date) - new Date(a.date)).slice(0, 10);
        accordionsHtml += `<details class="mb-3 group [&_summary::-webkit-details-marker]:hidden"><summary class="cursor-pointer flex justify-between items-center mb-2 select-none border-b border-[var(--card-border)] pb-2"><span class="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path></svg> ВЫПОЛНЕНО И ЗАКРЫТО (${archiveTasks.length})</span><span class="text-slate-400 group-open:rotate-180">▼</span></summary><div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 pb-2">${recentArchive.map(t => renderCard(t, false, true)).join('')}</div></details>`;
    }

...
function updateFabButton(tabId) {
    ...
    if (tabId === 'tab-analytics') {
        ...
        // Смотрим, какая подвкладка открыта
...
function handleFabDownload() {
    ...
    // 1. Если мы на вкладке Подрядчиков - открываем красивое меню
    ...
    // 2. Для остальных вкладок - формируем PDF напрямую
    ...
    setTimeout(() => {
        if(loaderText) loaderText.innerText = "Формирование отчета...";
        try {
            if (ctx === 'sub-rating') exportPdfRating(data);
            else if (ctx === 'sub-onepager') exportPdfOnePager(data);
            else if (ctx === 'sub-data') exportPdfData(data);
            else if (typeof exportPdfEngineering === 'function' && ctx === 'sub-engineering') exportPdfEngineering(data);
            else showToast("Функция выгрузки для этой вкладки в разработке");
        } catch (e) {

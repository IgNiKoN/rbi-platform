...
## Фикс: пропала градиентная раскраска на плитке «Ур. качества (физика/докум.)» вкладки «Сводка» (2026-07-17)

* Контекст: после фидбека пользователя «пропало цветное выделение цифр уровня качества» — проверено headless (Chromium, seed-данные с низким/средним/высоким УрК, light+dark темы) во всех местах, где применяется `_urkGradientColor()` (карточки подрядчиков, дашборд-плитки, рейтинг на «Сводке», Глобальная сводка Компании) — раскраска работает корректно везде, кроме одного места.
* Найдена причина: НЕ регрессия от текущей оптимизации (индексы/пагинация/агрегат) — предсуществующая недоработка из более раннего шага (см. запись «Два индекса УрК — объединение плиток…» выше, 2026-07-16), где плитка «Ур. качества (физика/докум.)» в объектной «Сводке» (`renderOnePagerSubTab`) была объединена из двух плиток, но осталась со статичными цветами (`text-slate-800 dark:text-white` для физики, `text-indigo-600`/`text-slate-400` для документарного значения) вместо градиентной раскраски `_urkGradientColor()`, применённой во всех соседних местах (карточки подрядчиков, Глобальная сводка).
* Правка (`js/modules/quality/features/analytics/analytics.render.js`, `renderOnePagerSubTab`): оба числа плитки (`currAvgUrk` и `currAvgDoc`) переведены на `style="color:${_urkGradientColor(...)}"`, как везде — `—`-заполнитель при отсутствии документарного значения остаётся серым (`text-slate-400`).
* Проверки: `node --check` — 0 ошибок. `ReadLints` — 0 ошибок. Headless-скриншот вкладки «Сводка» с сид-данными (40%/65-70% диапазон) — оба числа корректно окрашены в оранжевый по градиенту.
* SERVICES_API.md: не затронут — точечная визуальная правка существующей разметки.
* Риски: минимальные — используется уже проверенный хелпер `_urkGradientColor()`, других мест с той же недоработкой не найдено.
